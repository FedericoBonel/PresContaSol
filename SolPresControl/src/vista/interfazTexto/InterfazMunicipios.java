 ...
 */
public class InterfazMunicipios {

    /**
     * Interfaz del gestor de Municipios, imprime municipios segun usuario, toma opcion, la valida y la devuelve
     */
    public static int municipiosInterfaz(Usuario usuario, Hashtable<String, Municipio> municipios) {
        ...
        if (usuario instanceof Cuentadante cuentadante) {
            ...
            if (cuentadante.hasMunicipio()) InterfazInformacion.imprimirMunicipio(cuentadante.getMunicipio());
        // De lo contrario imprime todos los municipios
        } else {
            for (String identificador : municipios.keySet())
                InterfazInformacion.imprimirMunicipio(municipios.get(identificador));
    ...
    public static int modificarMunicipioInterfaz() {
        ...
        return InterfacesGenerales.interfazImprimirTodoYPedirEntero(new String[]{TextosConstantes.MUNICIPIOS_MODIFICAR_CATEGORIA,
